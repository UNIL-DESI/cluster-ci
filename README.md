...
9. **Visualisation Interactive et Intégrité P2P (Live & Historique)** : Le Live DVC-Viewer partage le namespace PID du conteneur d'exécution principal (`--pid=container`), ce qui lui permet d'afficher en temps réel le nœud exact de la pipeline en cours de calcul. L'Historique DVC-Viewer supporte un isolement strict via un paramètre de requête `?mode=history` (pour ne pas être écrasé par un run actif) et orchestre un pull DVC P2P en arrière-plan en ciblant dynamiquement le worker d'origine ayant produit la révision demandée, garantissant que l'arbre affiché contient toujours les derniers artefacts physiques.
